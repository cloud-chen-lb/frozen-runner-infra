...
# 重要變數：CLOUD_BUILD_SOURCE_BRANCH、*_VPC_ARGS、*_SECRET_MAPPING、*_RUNTIME_ENV_VARS、APP_* resource overrides。
...
if gcloud builds triggers describe "${PRODUCTION_TRIGGER_NAME}" \
  ...
  for key in _IMAGE_TAG _REGION _REPOSITORY _APP_IMAGE _MIGRATION_IMAGE _APP_SERVICE _MIGRATION_JOB \
    _DEPLOYER_SERVICE_ACCOUNT _APP_SERVICE_ACCOUNT _MIGRATOR_SERVICE_ACCOUNT _APP_VPC_ARGS \
     _MIGRATION_VPC_ARGS _APP_SECRET_MAPPING _MIGRATION_SECRET_MAPPING _APP_RUNTIME_ENV_VARS _MIGRATION_RUNTIME_ENV_VARS \
     _APP_MIN_INSTANCE _APP_MAX_INSTANCE _APP_CPU _APP_MEMORY _APP_TIMEOUT _APP_CONCURRENCY; do
    expected="${key#_}"
    case "${key}" in
      _IMAGE_TAG) expected="";;
      ...
      _APP_MIN_INSTANCE) expected="${APP_MIN_INSTANCE}";;
      _APP_MAX_INSTANCE) expected="${APP_MAX_INSTANCE}";;
      _APP_CPU) expected="${APP_CPU}";;
      _APP_MEMORY) expected="${APP_MEMORY}";;
      _APP_TIMEOUT) expected="${APP_TIMEOUT}";;
      _APP_CONCURRENCY) expected="${APP_CONCURRENCY}";;
    esac

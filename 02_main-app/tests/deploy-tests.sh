...
EOF
  ...
  grep -F -- ';_APP_RUNTIME_ENV_VARS=NEXT_PUBLIC_APP_DEFAULT_LOCALE=zh-TW' "${log}"
  grep -F -- ';_MIGRATION_RUNTIME_ENV_VARS=' "${log}"
}

test_service_account_contract() {
  local yaml
  yaml="/Users/leadbest/Documents/Work/frozen-runner/cicd/prod/cloudbuild-deploy.yaml"

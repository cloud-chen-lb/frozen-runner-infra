...
bash 02_main-app/scripts/02_create-postgres-instance.sh
bash 02_main-app/scripts/03_create-postgres-database-users.sh
bash 03_co-signer/scripts/04_create-mysql-instance.sh
```

每個商戶在建立 VM 前建立自己的 MySQL database/user：

```bash
printf '%s\n' '<PASSWORD>' | bash 03_co-signer/scripts/05_create-mysql-database-user.sh --merchant-slug=merchant-a
```

建立完成後，可查詢 private IP 並輸出不含真實密碼的 connection strings：

```bash
bash 02_main-app/scripts/04_print-connection-strings.sh
```

...
建立 connection/repository 後，將名稱填入 `02_main-app/scripts/env/env.sh`。不要填入完整的 `projects/...` resource path；`build-image` 腳本會依據 `GOOGLE_PROJECT_ID`、`GOOGLE_PROJECT_REGION` 與這兩個名稱自動組合完整路徑。
...
- `02_main-app/scripts/env/env.sh` 只放 Cloud Build connection/repository metadata。

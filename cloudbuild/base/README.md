...
```bash
bash cloudbuild/base/scripts/01_enable-apis.sh
```

執行人 IAM role：

```bash
bash cloudbuild/base/scripts/01_setup-exec-iam-account-role.sh
```

### 2. 建立 Cloud Build service account

建立 `cb-share-build`，並授予 Artifact Registry push、Cloud Build 與 Logging 所需權限：

```bash
bash cloudbuild/base/scripts/02_setup-cloud-build-iam.sh
```

### 3. 建立 GitHub connection 與 repository

這一步需要 Cloud Build GitHub App 的互動式 OAuth / installation。若由其他負責人建立，可略過；未來自行負責時執行：

```bash
bash cloudbuild/base/scripts/03_setup-github-connection.sh \
  CONNECTION_NAME \
  REPOSITORY_NAME \
...
建立 connection/repository 後，將名稱填入 `cloudbuild/base/scripts/env/env.sh`。不要填入完整的 `projects/...` resource path；`build-image` 腳本會依據 `GOOGLE_PROJECT_ID`、`GOOGLE_PROJECT_REGION` 與這兩個名稱自動組合完整路徑。
...
- `cloudbuild/base/scripts/env/env.sh` 只放 Cloud Build connection/repository metadata。
- 不要把 GitHub token、service-account JSON key 或其他 secret 寫入 env 檔案。
- `03_setup-github-connection.sh` 只處理 connection/repository，不會自動回填 env，避免覆蓋人工設定。

...
- `00_create-ci-trigger.sh` 只建立或確認 CI trigger；預設 PR target branch 是 `master`，可用 `CLOUD_BUILD_SOURCE_BRANCH` 覆寫。
- `03_run-release-trigger.sh` 只會觸發既有 automatic trigger，不會建立 trigger；首次使用前必須先執行 `01`、`00` 與 `02`。
- `05_create-manual-release-trigger.sh` 只建立純手動 trigger；`06_run-manual-release-trigger.sh` 只執行既有的純手動 trigger。

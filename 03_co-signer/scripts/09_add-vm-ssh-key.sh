...
if ! gcloud compute firewall-rules describe "${FIREWALL_RULE_NAME}" --project="${GOOGLE_PROJECT_ID}" \
  >/dev/null 2>&1; then
  gcloud compute firewall-rules create "${FIREWALL_RULE_NAME}" --project="${GOOGLE_PROJECT_ID}" \
    --network="${NETWORK_NAME}" --direction=INGRESS --action=ALLOW --rules=tcp:22 \
    --source-ranges="${VM_SSH_SOURCE_CIDR}" --target-service-accounts="${VM_SERVICE_ACCOUNT_EMAIL}" \
    --description='Temporarily allows SSH TCP/22 to the merchant Co-Signer VM from the configured operator CIDR and is removed by 10_remove-vm-ssh-key.sh.'
fi
...
if [[ -n "${existing_metadata}" ]]; then
  printf '%s\n%s\n' "${existing_metadata}" "${PUBLIC_KEY}" >"${metadata_file}"
else
  printf '%s\n' "${PUBLIC_KEY}" >"${metadata_file}"
fi

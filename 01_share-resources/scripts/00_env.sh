...
for variable in PROJECT_NAME GOOGLE_PROJECT_ID GOOGLE_PROJECT_REGION EXEC_IAM_ACCOUNT \
  MAIN_APP_SUBNET_CIDR COSIGNER_SUBNET_CIDR PRIVATE_SERVICES_RANGE_CIDR; do
  if [[ -z "${!variable:-}" ]]; then
...
if [[ ! "${GOOGLE_PROJECT_REGION}" =~ ^[a-z][a-z0-9-]*[0-9]$ ]] || \
  ! valid_cidr "${MAIN_APP_SUBNET_CIDR}" || ! valid_cidr "${COSIGNER_SUBNET_CIDR}" ||
  ! valid_cidr "${PRIVATE_SERVICES_RANGE_CIDR}"; then
...
export MAIN_APP_SUBNET_CIDR COSIGNER_SUBNET_CIDR PRIVATE_SERVICES_RANGE_CIDR
export NETWORK_NAME MAIN_APP_SUBNET_NAME COSIGNER_SUBNET_NAME PRIVATE_SERVICES_RANGE_NAME
